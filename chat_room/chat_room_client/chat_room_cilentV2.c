...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        fprintf(stderr, "usage: client hostname token\n");
        exit(1);
    }
    ...
    {
        char authMsg[MAXDATASIZE];
        int authLen = snprintf(authMsg, sizeof(authMsg), "AUTH %s\n", token);
        if (authLen < 0 || (size_t)authLen >= sizeof(authMsg)) {
            fprintf(stderr, "client: token too long\n");
            close(sockfd);
            return 1;
        }
        if (send(sockfd, authMsg, (size_t)authLen, 0) < 0) {
            perror("client: send auth");
            close(sockfd);
            return 1;
        }
    }
